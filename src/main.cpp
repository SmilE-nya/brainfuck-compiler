...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        std::string a = argv[i];
        if (a == "-v") { std::cout << "bfc v1.0.0\n"; return 0; }
        else if (a == "-o") { if (++i < argc) out = argv[i]; else goto u; }
        else if (a == "--emit" && ++i < argc) { char m = *argv[i];
            emit = m=='t'?0:m=='c'?1:m=='e'?2:(std::cerr<<"bad\n",-1);
            if (emit < 0) return 1; }
        else if (a[0] == '-') { std::cerr << "?\n"; return 1; }
        else in = a;
    }
    if (in.empty()) { u:
    ...
    for (int i = 0; i < (int)src.size(); ++i) {
        if (is_bf(src[i])) { tok += src[i]; pos.push_back(i); }
    }

    if (emit == 0) { std::cout << tok; return 0; }
    ...
    // 收集全局错误，排序后输出（按行、列顺序）
    struct Err { int l, c; std::string msg; };
    std::vector<Err> errs;
    ...
    for (int i = 0; i < (int)tok.size(); ++i) {
        switch (tok[i]) {
            ...
            case '+': cpp += "  tape[ptr]+=1;\n"; break;
            case '-': cpp += "  tape[ptr]-=1;\n"; break;
            case '.': cpp += "  std::cout<<tape[ptr];\n"; break;
            case ',': cpp += "  tape[ptr]=(unsigned char)std::cin.get();\n"; break;
            case '[': st.push_back(i); cpp += "  while(tape[ptr]){\n"; break;
            case ']':
                {
                    int l, c; loc(pos[i], l, c);
                    if (st.empty())
    ...
    for (int s : st) { int l, c; loc(pos[s], l, c); errs.push_back({l, c, "unmatched '['"}); }
    ...
    o << cpp; o.close();
    int r = std::system(("g++ -O2 -o " + (out.empty()?"a":out) + " _t.cpp").c_str());
    std::remove("_t.cpp");
    if (r) { std::cerr << "compilation error (see above)\n"; return 1; }
    std::cout << "OK\n"; return 0;
}